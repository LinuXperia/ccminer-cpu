...
char *Platform::createUserAgent()
{
    ...
    int length = snprintf(buf, max, "%s/%s (Windows NT %lu.%lu", "ccminer-amd", "0.1", osver.dwMajorVersion, osver.dwMinorVersion);
...
#   elif _MSC_VER
    length += snprintf(buf + length, max - length, " msvc/%d", _MSC_BUILD);
#   endif
