...
static void *miner_thread(void *userdata)
{
	...
	int thr_id = mythr->id;
	void* threadInfo = mythr->threadInfo;
	struct work work;
	uint64_t loopcnt = 0;
	uint32_t max_nonce;
	...
	while (!abort_flag) {
		...
		switch (opt_algo) {
		case ALGO_KEVA:
			rc = scanhash_cryptonight_keva(thr_id, threadInfo, &work, max_nonce, &hashes_done, 2);
			break;
...
int main(int argc, char *argv[])
{
	...
	g_controller = new xmrig::Controller();
	if (g_controller->init(0, nullptr) != 0) {
